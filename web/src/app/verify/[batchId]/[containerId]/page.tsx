...
export default function VerificationPage() {
  ...
  useEffect(() => {
    async function recordScan() {
      try {
        const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'https://sih-2026-kiit-1c9fdv5nf-rehan-sumans-projects.vercel.app';
        ...
        <div className={`rounded-t-3xl flex-shrink-0 flex flex-col relative ${currentGradient}`}>
           ...
           <div className="px-6 py-4 border-t border-white/20 mt-3 flex items-center">
              ...
              <div className="flex-1 min-w-0">
                 ...
                 <p className="text-white font-black text-xl truncate drop-shadow-md">{data?.hive?.location || 'Unknown'}</p>
              </div>
           </div>
           ...
           <div className="px-6 pb-5 pt-2 flex justify-between">
              <div>
                 ...
                 <p className="text-white font-bold text-base drop-shadow-sm">{data?.harvestDate ? new Date(data.harvestDate).toLocaleDateString(undefined, {month: 'short', day: 'numeric'}) : '--'}</p>
              </div>
              <div className="text-center">
                 <p className="text-white/80 text-[10px] font-black uppercase tracking-widest mb-0.5 drop-shadow-sm">Quality</p>
                 <p className="text-white font-bold text-base drop-shadow-sm">{isVerified ? 'PASSED' : 'FAIL'}</p>
              </div>
           ...
           <div className="px-6 pb-6 text-center">
             <div className="inline-flex items-center justify-center bg-black/25 backdrop-blur-md rounded-xl px-4 py-3 w-full shadow-inner border border-white/10">
               ...
               <p className="text-white text-sm font-bold truncate">{isSuspicious ? riskReason : isVerified ? 'Verified Authentic Product' : 'Invalid Code'}</p>
             </div>
           </div>
        </div>
        ...
        <div className="bg-white rounded-b-3xl px-6 pt-6 pb-6 flex-1 flex flex-col shadow-inner border-t-0 overflow-y-auto">
             ...
             <div className="w-full flex-1 mb-4">
               ...
               <div className="space-y-3">
                 {data.events.slice(-3).map((evt: any, idx: number) => (
                   <div key={idx} className="flex items-start text-sm bg-[#fafafa] p-3 rounded-xl border border-gray-100 transition-all hover:bg-gray-50">
                     ...
                     <div className="flex-1 min-w-0">
                       ...
                       <p className="text-[10px] text-gray-500 mt-0.5">{new Date(evt.timestamp).toLocaleString(undefined, {month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit'})}</p>
                     </div>
                   </div>
                 ))}
               </div>
           ...
           {data?.txHash && (
             <div className="w-full mt-auto bg-gray-900 rounded-2xl p-4 flex flex-col items-center shadow-lg border border-gray-800">
                ...
                <a href={`https://amoy.polygonscan.com/tx/${data.txHash}`} target="_blank" rel="noreferrer" className="text-[10px] font-black text-gray-900 bg-white hover:bg-gray-200 transition-colors px-5 py-2 rounded-full uppercase tracking-widest flex items-center shadow-md">
                  Verify Explorer <ChevronRight className="w-3.5 h-3.5 ml-1" />
                </a>
             </div>
           )}
